...
export default function DecisionBanner({ decision, quote, halalStatus, notionSaved }) {
  ...
  return (
    <div className={`card p-6 ${cfg.bg} border ${cfg.border} animate-fade-in`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <Icon className={cfg.iconColor} size={36} strokeWidth={1.5} />
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className={`font-display text-4xl font-bold ${cfg.textColor}`}>{cfg.label}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${cfg.badge}`}>
                {decision.confidence} confidence
              </span>
            </div>
            <p className="text-slate-600 text-sm">{decision.reason}</p>
          </div>
        </div>

        <div className="text-right shrink-0">
          <div className="text-2xl font-display font-bold text-slate-900">${quote?.price?.toFixed(2)}</div>
          <div className="text-slate-400 text-xs mb-2">{quote?.ticker}</div>
